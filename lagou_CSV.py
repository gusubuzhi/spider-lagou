...
def get_page(keyworld,city):
    data = {
        'first': 'false',
        'pn': 2,
        'kd': keyworld,
        }
    url = "https://www.lagou.com/jobs/positionAjax.json?px=new&city=%s&needAddtionalResult=false" %city
...
def save_first_page(keyworld,city):
    data = {
        'first': 'true',
        'pn': 1,
        'kd': keyworld,
    ...
    data = res.json()
    itme = pare_data(data)
    save_to_csv(itme, keyworld,city)

def get_page_data(page,keyworld,city):
    data = {
        'first': 'false',
        'pn': page,
        'kd': keyworld,
    ...
    detailcity = city
    url = "https://www.lagou.com/jobs/positionAjax.json?px=new&city=%s&needAddtionalResult=false" % detailcity
    res = requests.post(url, data=data, headers=headers)
    data = res.json()
    return data
...
def save_to_csv(itme,keyworld,city):
    fieldpath = "%s-%s.csv" %(keyworld,city)
...
def main():
    while True:
        ...
        start_data = get_page(keyworld,city)
        data_num =get_data_num(start_data)
        page_num = data_num //15
        print("********一共有【%d】条数据, 共【%d】页********" % (data_num,page_num))
        save_first_page(keyworld, city)
        for page in range(2,page_num+1):
            data = get_page_data(page,keyworld,city)
            itme=pare_data(data)
            try:
                save_to_csv(itme,keyworld,city)
            except UnicodeEncodeError:
                pass
        # print(r"++++ 文件存储在 D:\Bin 文件夹下面+++")
